...
function generateSuffixDomains(name) {
  var new_name = name.split(" ").join("");
  var new_list = suffix_domain.map(function (d) {
    return `${new_name}.${d}`;
  });
...
function generatePrefixDomains(name) {
  var new_name = name.split(" ").join("");
  var new_list = prefix_domain.map(function (d) {
    return `${new_name}.${d}`;
  });
...
function generateExtraDomains(name) {
  var new_name = name.split(" ").join("");
  var new_list = extra_domain.map(function (d) {
    return `${new_name}.${d}`;
  });
...
function generateAllDomains(name) {
  ...
  var common_list = common_domain.map(function (d) {
    return `${new_name}.${d}`;
  });
  var extra_list = extra_domain.map(function (d) {
    return `${new_name}.${d}`;
  });
  var prefix_list = prefix_domain.map(function (d) {
    return `${new_name}.${d}`;
  });
  var suffix_list = suffix_domain.map(function (d) {
    return `${new_name}.${d}`;
  });
  var all_list = {
    "common domains": common_list,
    "extra domains": extra_list,
    "new domains": new_list,
    "prefix domains": prefix_list,
    "suffix domain": suffix_list,
  };
...
function generateDomains(name) {
  ...
  console.log(
    chalk.white.bgBlue.bold("Showing Common Domain Names:: "),
    chalk.white.bgBlue.bold(new_name)
  );
...
program
  ...
  .description("Get Common Domain Names and Save to JSON File")
  .alias("g")
  .action((name) => {
    generateDomains(name);
  });
program
